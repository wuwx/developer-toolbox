<template>
  <UContainer class="py-8 sm:py-12">
    <UPageHeader
      title="UUID v3 生成器"
      description="基于命名空间和名称的 UUID (Version 3, MD5)"
      align="center"
    >
      <template #icon>
    ...
    <div class="flex flex-wrap justify-center gap-2 mb-8">
      ...
      <UButton to="/uuid/v3" color="primary" variant="solid" size="sm">UUID v3 (MD5)</UButton>
    ...
    <ToolInfo title="关于 UUID v3" :items="accordionItems" />
  </UContainer>
</template>
...
const generateUUIDs = () => {
  ...
  names.forEach(name => {
    try {
      uuids.value.push({
        name: name.trim(),
        uuid: uuidv3(name.trim(), namespace.value)
      })
    } catch (e) {
      console.error(e)
    }
...
const accordionItems: AccordionItem[] = [
  {
    ...
    content: 'UUID Version 3 是基于命名空间（Namespace）和名称（Name）生成的。它使用 MD5 哈希算法。对于相同的 Namespace 和 Name，生成的 UUID 总是相同的。'
  },
  {
    slot: 'usage',
    label: '主要用途',
    icon: 'i-heroicons-rocket-launch',
    content: '当需要根据某些现有标识符（如域名、URL、用户名）生成固定的 UUID 时使用。这保证了在不同系统间的一致性。'
  }
]
...
useHead({
  title: 'UUID v3 生成器 - 基于 MD5 的命名空间 UUID',
  meta: [
    { name: 'description', content: '在线生成 UUID v3，基于 Namespace 和 Name 的 MD5 哈希生成。' }
  ]
})
</script>
