<template>
  <UContainer class="py-8 sm:py-12">
    <UPageHeader :title="$t('pages.urlParser.title')" :description="$t('pages.urlParser.description')" align="center">
      <template #icon>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="font-semibold">{{ $t('ui.input') }} URL</h3>
            <div class="flex gap-2">
              <UButton color="neutral" variant="soft" size="xs" icon="i-heroicons-clipboard-document" @click="copyToClipboard(urlInput, 'URL')">{{ $t('ui.copy') }}</UButton>
              <UButton color="neutral" variant="ghost" size="xs" icon="i-heroicons-x-mark" @click="urlInput = ''">{{ $t('ui.clear') }}</UButton>
            </div>
          </div>
        </template>
        <UInput v-model="urlInput" placeholder="https://example.com/path?query=1" size="lg" class="font-mono" />
      </UCard>

      <div v-if="parsedUrl" class="grid lg:grid-cols-2 gap-6">
        <UCard>
          <template #header><h3 class="font-semibold">URL Components</h3></template>
          <div class="space-y-4">
            <div>
              <label class="text-xs font-medium text-gray-500">Protocol</label>
              <USelect v-model="components.protocol" :options="[{label: 'https:', value: 'https:'}, {label: 'http:', value: 'http:'}, {label: 'ftp:', value: 'ftp:'}, {label: 'ws:', value: 'ws:'}, {label: 'wss:', value: 'wss:'}]" size="sm" class="w-full" />
            </div>
            <div>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              <h3 class="font-semibold">{{ $t('ui.queryParams') }}</h3>
              <UButton color="primary" variant="soft" size="xs" icon="i-heroicons-plus" @click="addParam">{{ $t('ui.add') }}</UButton>
            </div>
          </template>
          <div class="space-y-2 max-h-[400px] overflow-y-auto pr-1">
            <div v-if="components.params.length === 0" class="text-sm text-gray-400 text-center py-4">{{ $t('ui.noQueryParams') }}</div>
            <div v-for="(param, index) in components.params" :key="index" class="flex gap-2">
              ...
              <UButton color="error" variant="ghost" icon="i-heroicons-trash" size="xs" @click="removeParam(index)" />
            </div>
          </div>
        </UCard>
      </div>
...
useHead({ title: t('pages.urlParser.title'), meta: [{ name: 'description', content: t('pages.urlParser.description') }] })
</script>
