<template>
  <UContainer class="py-8 sm:py-12">
    <UPageHeader :title="$t('pages.jsonDiff.title')" :description="$t('pages.jsonDiff.description')" align="center">
      <template #icon>
    ...
    <UCard class="mt-8">
      <template #header><h3 class="font-semibold">{{ $t('ui.differences') }}</h3></template>
      <UButton class="mb-4" color="primary" @click="compare">{{ $t('ui.compare') }}</UButton>
...
useHead({title:t('pages.jsonDiff.title'),meta:[{name:'description',content:t('pages.jsonDiff.description')}]})
</script>
