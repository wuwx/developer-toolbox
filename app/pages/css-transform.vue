<template>
  <UContainer class="py-8 sm:py-12">
    <UPageHeader title="CSS Transform" description="CSS 变换生成器" align="center">
      <template #icon>
    ...
    <div class="grid lg:grid-cols-2 gap-8">
      <div class="space-y-6">
        <UCard>
          <template #header><h3 class="font-semibold">变换属性</h3></template>
          <div class="space-y-4">
            <div>
              <label class="text-sm font-medium mb-2 block">旋转: {{rotate}}deg</label>
              <URange v-model="rotate" :min="-180" :max="180"/>
            </div>
            <div>
              <label class="text-sm font-medium mb-2 block">缩放: {{scale}}</label>
              <URange v-model="scale" :min="0.5" :max="2" :step="0.1"/>
            </div>
            <div>
              <label class="text-sm font-medium mb-2 block">X偏移: {{translateX}}px</label>
              <URange v-model="translateX" :min="-100" :max="100"/>
            </div>
            <div>
              <label class="text-sm font-medium mb-2 block">Y偏移: {{translateY}}px</label>
        ...
        <UCard>
          <template #header>
            <div class="flex justify-between items-center">
              ...
              <UButton color="primary" variant="soft" size="sm" @click="copyToClipboard(css,'CSS')">复制</UButton>
            </div>
          </template>
...
useHead({title:'CSS Transform | 开发者工具箱'})
</script>
